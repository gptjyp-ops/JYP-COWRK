...
def foreign_streak_candidates():
    ...
    rows = fetch(
        ...
        "for_cont_nettrde_upper", max_pages=3, max_rows=CANDIDATE_LIMIT,
    )
    out = []
    for r in rows:
...
def investor_5d(code: str):
    # ka10059 종목별투자자기관별요청: 최근 5거래일 순매수 수량(단주)
    ...
    f5 = sum(num(r.get("frgnr_invsr")) for r in rows[:5])
    i5 = sum(num(r.get("orgn")) for r in rows[:5])
    p5 = sum(num(r.get("ind_invsr")) for r in rows[:5])
    return f5, i5, p5


def chart_metrics(code: str):
    # ka10081 일봉: 현재가, 20/60일선, 거래량 배수
    today = datetime.now().strftime("%Y%m%d")
    rows = fetch(
    ...
    ma60 = sum(prices[:60]) / min(60, len(prices)) if prices else 0
    prev20 = vols[1:21]
    avg20vol = sum(prev20) / len(prev20) if prev20 else 0
    vol_ratio = (vols[0] / avg20vol) if vols and avg20vol else 0
    dist20 = ((current / ma20) - 1) * 100 if ma20 else 0
    dist60 = ((current / ma60) - 1) * 100 if ma60 else 0
    change = 0.0
    if len(prices) >= 2 and prices[1]:
        change = (current / prices[1] - 1) * 100
...
def main():
    print("[1/3] 외국인 연속순매수 후보 수집")
    candidates = foreign_streak_candidates()
    ...
    payload = {
        ...
        "version": "stock-miner-collector-v1",
        "note": "키움 1차 발굴 점수. DART 실적/공시 점수는 다음 단계에서 추가.",
        "count": len(good),
        "stocks": good,
